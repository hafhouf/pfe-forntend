...
const Form = () => {
  ...
  const fetchCampaigns = async () => {
    try {
      const response = await axios.get('http://localhost:8086/api/campaigns');
  ...
  const handleSubmit = async (e) => {
    ...
    const form = { title, description, questions, campaign: { id: selectedCampaign } };
    try {
      await axios.post(`http://localhost:8086/api/forms/${user.id}`, form);
      toast.success('Form created successfully');
      // Clear the form fields after successful submission
    ...
    <>
      ...
      <Container className="py-5">
        ...
        <BootstrapForm onSubmit={handleSubmit}>
          ...
          <BootstrapForm.Group controlId="formCampaign" className="mb-3">
            <BootstrapForm.Label>Campaign</BootstrapForm.Label>
            <BootstrapForm.Control
              as="select"
